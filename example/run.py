...
pn.convert_bin_to_las('data/bin', deleteBin=True)
pn.rotate_crop_filter_pcl(project_dir='data')
pn.las_to_tif(project_dir='data')
pn.tif_to_netcdf(project_dir='data')

# logic to remove all intermediate files
for folder in ['las_raw', 'las_clean', 'TIFs']:
    flist = glob.glob('data' + os.sep + folder + os.sep + '*')
    for file in flist:
        os.remove(file)
...
es.define_summer_surface(ds, '2022-07-15', '2022-07-16')
es.remove_outliers(ds)
es.median_spacetime_filtering(ds)
es.surface_to_depth(ds, 'data/sdepths.nc')
