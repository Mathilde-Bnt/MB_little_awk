...
def get_met_forcing(simulation_start_date='2021-12-06 00:00:00', file_start_date='2021-12-01 00:00:00',
    ...
    with open(file_name) as met_data:
        ...
        header = next(reader)    # TODO get index of rows automatically? (in any case, useful for skipping first line)
        ...
        if accumulation_index<len(end_accumulation_times) and i*dt>=end_accumulation_times[accumulation_index]:     # if an accumulation was passed
            ...
            if detect_ice:
                if accumulation_index+1 >= len(start_accumulation_times) and erosion_index >= len(start_erosion_times):   # got to the end
                    # TODO Nope c'est pas la bonne condition, faut separer plus de cas
...
def simulate_snowpack_evolution_icy(ds, x_sel, y_sel, nb_iterations, end_accumulation_times, end_erosion_times,
                                start_accumulation_times, start_erosion_times, start_accumulation, end_accumulation,
                                start_erosion, end_erosion, jj, dt, ro_layer, ro_water, ro_ice, t_old, tf, tsfc_default,
                                dy_snow, age_layers, gamma, cp_snow, melt_flag, a1_vector, a1_value, a2, slope_threshold,
                                min_duration_in_s, met_temp_data=[None], met_wind_data=[None], met_time_data=[0], 
                                fit_top_of_snowfall_to_curve=False, erode_several_layers=False):
    '''
    Function that simulates the evolution of the snowpack over a certain period of time
    # TODO
    Args:
        ...
        a1_vector: array containing the a1 values (m^-1.s^-1) for each layers (null for ice layers that do not compact)
        a1_value: exponential parameter, empirically calibrated, default value of a1 for new layers (m^-1.s^-1) 
        a2: exponential parameter, empirically calibrated (m^3.kg^-1)
        ...
        met_temp_data: array containing surface temperatures for each meteorological timestamp (degrees Celcius),
                default [None], i.e. use tsfc=tsfc_default
        met_wind_data: array containing wind speed for each meteorological timestamp (m.s^-1),
                default [None], i.e. use new_snow_ro=150
        met_time_data: array containing the meteorological timestamps (s since start of simulation), default [0]

        fit_top_of_snowfall_to_curve: boolean, if True the height of snowfalls will be such that the snow depth matches
                the lidar curve depth at the end of snowfalls, default False
        erode_several_layers: boolean, if True the erosions may affect more than just the top layer
                (remove the measured snow height difference from the simulated layers), default False
    ...
    # TODO unify previous and current functions?

    # Initialize arrays to keep track of variables in time
    ro_layer_evolution, depth_evolution, temperature_evolution, ice_layers_times = [], [], [], []
    ...
    for i in range(nb_iterations):
        ...
        # Update surface temperatur if needed
        if temperature_index<len(met_temp_data) and met_time_data[temperature_index]!=None and i*dt>=met_time_data[temperature_index]:
            if met_temp_data[temperature_index] != None:
                tsfc = float(met_temp_data[temperature_index])
            temperature_index += 1
