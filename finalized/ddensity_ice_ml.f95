subroutine ddensity_ml(ro_layer, Tf, dt, ro_water, ro_ice, T_old, JJ, dy_snow, A1, A2, nz_max, ro_layer_out, dy_snow_out)
    ...
    real, intent(in)    :: Tf, dt, ro_water, ro_ice, A1(nz_max), A2, T_old(nz_max)    ! A1 is a vector here
    ...
    if (JJ.gt.0) then     ! JJ is the number of layers
        ...
        do j=1, JJ
            ...
            ro_layer_out(j) = ro_layer(j) + dt * A1(j)*sweqstar(j)*ro_layer(j) * exp(-0.08*(Tf-T_old(j))) * exp(-A2*ro_layer(j))
            ! For layers with a null A1 (ice layers), no compaction occurs
